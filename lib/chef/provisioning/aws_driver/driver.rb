...
def ready_machine(action_handler, machine_spec, machine_options)
    ...
    machine_spec.reference['ready_at'] = Time.now.utc.to_s
    end

    if machine_spec.reference['is_windows'] and not machine_spec.reference['ready_at']
    # windows needs running AND ready (available via console)
    # so we need to wait_until_ready EVEN when the machine is already running
    wait_until_ready_machine(action_handler, machine_spec, instance)
    machine_spec.reference['ready_at'] = Time.now.utc.to_s
    end
...
def bootstrap_options_for(action_handler, machine_spec, machine_options)
    ...
    # setting default windows user_data
...
def create_winrm_transport(machine_spec, machine_options, instance)
    ...
    transport_type = machine_options[:winrm_transport] || 'https'
    ...
    def wait_until_ready_machine(action_handler, machine_spec, instance=nil)
      wait_until_machine(action_handler, machine_spec, "be ready to use", instance) { |instance, machine_spec|
        if instance.platform == "windows"
          # windows can reboot a few times, so we need running AND a ready
          # messages on the console
          if instance.state.name == "running" && instance.console_output.data.output
            Base64.decode64(
